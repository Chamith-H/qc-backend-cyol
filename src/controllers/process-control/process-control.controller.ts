...
export class ProcessControlController {
  ...
  @Post('shift-expand')
  async getShiftExpand(@Body() dto: SelectedShiftDto) {
    return await this.processControlService.get_selectedShiftExpand(dto);
  }
  ...
  @Post('change-status')
  async changeStatus(@Body() dto: UpdateProcessStatusDto) {
    return await this.processControlService.update_qcStatus(dto);
  }
}
